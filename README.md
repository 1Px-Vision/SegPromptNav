...
![](https://github.com/1Px-Vision/SegPromptNav/blob/main/mark_LLM.jpg)
...
The image below shows an outdoor street navigation example where the agent follows a planned path across an urban environment.

![](https://github.com/1Px-Vision/SegPromptNav/blob/main/waypoint_Q_learning.jpg)

The cyan trajectory represents the estimated navigation path generated by the system. The agent starts from the initial point, follows the waypoint sequence, and reaches the target location while adapting the route according to the street layout and visual information.

### System Architecture

The proposed navigation framework is composed of four main modules:
```
Camera + IMU
     |
     v
